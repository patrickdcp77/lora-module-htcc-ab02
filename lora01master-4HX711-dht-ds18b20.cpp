...
cordon blanc plat et fils couleurs pales
rose    E+
bleu    E-
blanc   AB-
jaune   AB+
...
/* état de la LED flash RGB en fonction des phases
...
/* ABP para*/ ///ne pas effacer car d'autres variables en dépandent
...
void setup() {
  ...
  pinMode(Vext, OUTPUT);
  digitalWrite(Vext, LOW);// pour activer l'alimentation des capteurs branchés sur Vext
  delay(500);
  
  float Sample_weight;

  //////////dans le SETUP on calcule une bonne fois l'offset en mesurant la balance à VIDE après un RESET
    //si reset, l'offset n'est pas sauvegardé dans une mémoire EPROM
    //donc si changement de pile, faire un reset balances vides pour avoir la bonne tare!!!!!!!!!!!
  Hx711_N1.begin(PIN_HX711_N1_DATA_OUT,PIN_HX711_N1_SCK_AND_POWER_DOWN,64 );
  ...
  // la fonction est appelée et redonne toutes les mesures 
  //en tenant compte de l'offset calculé plus haut
